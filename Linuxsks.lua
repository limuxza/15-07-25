...
function Linux:UpdateThemeColors()
    local accentColor = self.Themes[self.CurrentTheme]
    
    for _, gui in pairs(game:GetService("CoreGui"):GetChildren()) do
        if gui:IsA("ScreenGui") and gui.Name:match("^UI_%d+$") then
            local function updateElement(element)
                if element:IsA("Frame") and element.Name == "TabAccentBar" then
                    element.BackgroundColor3 = accentColor
                elseif element:IsA("Frame") and element.Name == "Fill" then
                    element.BackgroundColor3 = accentColor
                elseif element:IsA("Frame") and element.Name == "OptAccentBar" then
                    element.BackgroundColor3 = accentColor
                end
...
function Linux.Create(config)
    ...
    if config.ConfigSave ~= false then
        ...
        SettingsTab.Section({Name = "Audio", Container = 1})
        ...
        SettingsTab.Section({Name = "Theme", Container = 2})
        ...
        SettingsTab.Button({
            ...
            Container = 2,
            Callback = function()
                Linux.CurrentTheme = selectedTheme
                Linux:UpdateThemeColors()
                Linux:Notify({
                    Title = "Theme Applied",
